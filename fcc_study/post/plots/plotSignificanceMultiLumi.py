...
def getPlotGrid(mHs, diff_scan, all_signifs):
    ...
    return plot_grid, plot_grid_for_contour
...
plot_grid, plot_grid_for_contour = getPlotGrid(mHs, diff_scan, all_signifs)
...
for combine_direc_other, lumi_other in zip(combine_direc_others, lumi_others):
    ...
    # For a single mH, get the values of mA - mH
    diff_scan = all_ms[all_ms[:, 0] == mHs[0], 1] - mHs[0]
    plot_grid_other, plot_grid_for_contour_other = getPlotGrid(mHs, diff_scan, all_signifs_other)
...
for i, (lumi_other, plot_grid_other) in enumerate(other_plot_grids_for_contour.items()):
    ...
    con_other = plt.contour(plot_grid_other, np.array([5]) , linewidths=[2], extent=extent, origin='upper',colors=[color])
...
legend_elements += handles_con + handles_con_filled + line + handles_others
legend_names += ["5$\sigma$ Discovery", "Discoverable", '$M_H$ + $M_A$ = $\sqrt{s}$'] + labels_others
