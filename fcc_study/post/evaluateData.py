...
class EvaluateModel:
    ...
    def getProbsForEachMass(self, dataset, unique_masses):
        ...
        unique_masses_normal = mass_scaler.inverse_transform(unique_masses)
        mass_strings = self.convertMassesToString(unique_masses_normal)


        dataset_fields = ak.fields(dataset.data)

        probabilities = {}
        # Loop over all masses
        n_masses = len(unique_masses)
        for i, (mass, m_string) in enumerate(zip(unique_masses, mass_strings)):
            print(f"Processing mass {i+1}/{n_masses} = {((i+1) / n_masses) * 100:.2f}%")
            if m_string in dataset_fields:
                print(f"Mass {m_string} already in dataset, so skipping")
                continue
            
            print(f"Finding probabilities for mass = {mass} = {m_string}")
            ...
            probabilities[m_string] = copy.deepcopy(probs)

        for m_string, probs in probabilities.items():
            print(f"Adding probs for mass {m_string}")
            dataset.data[m_string] = ak.flatten(probs)
    ...
    def convertMassesToString(self, masses):
        ...
        for m, ms in zip(masses, masses_strings):
            print(f"{m} -> {ms}")
