...
for sig_proc in tqdm(sig_procs):
    ...
    for PROCESS in ['electrons', 'muons']:
        ...
        os.makedirs(f"{run_loc}/combine/{sig_proc}", exist_ok=True)
        with uproot.recreate(f"{run_loc}/combine/{sig_proc}/{sig_proc}_{PROCESS}.root") as f:
        ...
        for datacard in datacard_files:
            os.system(f"cp {datacard} {run_loc}/combine/{sig_proc}/.")
