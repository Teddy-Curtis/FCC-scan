...
base_run_dir = f"runs/e240NewestData/scenario_{scenario}"
run_loc = getRunLoc(base_run_dir)

# Only read in 10 signal files for now
train_files = glob.glob(f"/vols/cms/emc21/FCC/FCC-Study/Data/NewestDataComplete/ecom240/scenario{scenario}/awkward_files/train/*.parquet")
train_data = []
...
val_files = glob.glob(f"/vols/cms/emc21/FCC/FCC-Study/Data/NewestDataComplete/ecom240/scenario{scenario}/awkward_files/val/*.parquet")
val_data = []
...
with open(f"Data/NewestDataComplete/ecom240/backgrounds.json", "r") as f:
    backgrounds = json.load(f)
with open(f"Data/NewestDataComplete/ecom240/scenario{scenario}_sig_dict.json", "r") as f:
    signals = json.load(f)
...
def evaluateAllData(run_name, all_masses, scenario):
    files = glob.glob(f"/vols/cms/emc21/FCC/FCC-Study/Data/NewestDataComplete/ecom240/scenario{scenario}/awkward_files/{run_name}/*.parquet")
    data = []
    for file in files:
